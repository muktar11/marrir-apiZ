...
class InvoiceModel(Base, EntityBaseModel):
    ...
    checkout_id = mapped_column(String, nullable=True)
    description = mapped_column(String, nullable=True)
     # 🔥 ADD THIS
    reference = mapped_column(String(255), nullable=True)   # merchantTransactionId
    # Invoice identity
    ...
    checkout_id = mapped_column(String(255), nullable=True)
    ...
    billing_city = mapped_column(String(2), nullable=True)
    billing_street = mapped_column(String(2), nullable=True)
    billing_postcode = mapped_column(String(2), nullable=True)
    billing_state = mapped_column(String(2), nullable=True)

    integrity = mapped_column(String(2), nullable=True)
    cv_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("table_users.id"), unique=True, nullable=True
    )
    reserve_id = mapped_column(String(255), nullable=True)

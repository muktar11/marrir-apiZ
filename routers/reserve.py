...
async def get_reserved_cvs(
        ...
        """
        Retrieve all reserved CVs.
        - If `nationality` is provided, filter CVs by nationality.
        - Always include records where `selfsponsor=True`.
        - Include only records where `approved=True`.
        """

        # Base query: Join RecruitmentSetReserveModel with CVModel
        query = (
        ...
        # Apply filters:
        # - Approved = True
        # - Nationality match (if provided)
        # - OR Self Sponsor = True (always included regardless of nationality)
        if nationality:
            query = query.filter(
                or_(
                    and_(
                ...
                # No nationality filter — include only unapproved and selfsponsor
                query = query.filter(
                    or_(
                        RecruitmentSetReserveModel.approved == False,
                        RecruitmentSetReserveModel.selfsponsor == True
        ...
        return {
            "status_code": 200,
            "message": "Reserved CVs fetched successfully",
            "error": False,
            "count": len(data),
            "data": data,
        }
...
async def get_buyer_requests_for_recruiter(
    ...
    # Step 1: Validate recruiter exists
    ...
    # Step 2: Get all CVs reserved by this recruiter and requested by buyers
...
async def get_buyer_requests_for_sponsor(
    ...
    # Step 1: Validate recruiter exists
    recruiter = db.query(UserModel).filter(UserModel.id == promoter_id).first()
    if not recruiter:

...
def generate_invoice_pdf(invoice: InvoiceModel):
    path = f"{INVOICE_DIR}/{invoice.invoice_number}.pdf"
    ...
    c.drawString(50, 740, f"Date: {invoice.created_at.strftime('%Y-%m-%d')}")
    c.drawString(50, 720, f"Amount: {invoice.amount} {invoice.currency.upper()}")

    c.drawString(50, 690, "Billed To:")
    c.drawString(50, 670, invoice.card_holder or "-")
    c.drawString(50, 650, f"Country: {invoice.billing_country or '-'}")
    c.drawString(50, 630, f"Email: {invoice.billing_email or '-'}")
    c.drawString(50, 610, f"Phone: {invoice.billing_phone or '-'}")

    c.drawString(50, 570, "Payment Method:")
    c.drawString(
        50,
        550,
        f"{invoice.card_brand} **** {invoice.card_last4}"
        if invoice.card_last4 else "-"
    )

    c.drawString(50, 510, "Thank you for your payment.")
    c.drawString(50, 490, "Company: Marrir.com")
...
def download_invoice(
    ...
    if not invoice or not invoice.invoice_file:
        raise HTTPException(404, "Invoice not ready")

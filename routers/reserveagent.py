...
from schemas.reserveschema import ApproveReserveSchema, ApprovesReserveSchema, PrivateReserveCreateSchema
...
@agent_reserve_router.post("/employer/pending-reserves/employer-request/purchase", status_code=500)
...
@agent_reserve_router.post("/recruiter/pending-reserves/employer-request/purchase", status_code=500)
async def public_test_500():
    """
    Public endpoint that always returns 500.
    """
...
async def get_pending_reserves_agent(
    employee_id: str,
    db: Session = Depends(get_db_raw)
):
    """
    Return pending reserves created by AGENTS only.
    Conditions:
    ...
    pending_reserves = (
        ...
        .filter(
            ...
            RecruitmentAgentPrivateReserveModel.agent_id.isnot(None),
    ...
    return {
        "status_code": 200,
        "message": "Agent-created pending reservations fetched successfully",
        "error": False,
        "count": len(data),
        "data": data
    }
...
async def approve_private_reserve(
    ...
    try:
        ...
        employee_uuid = str(uuid.UUID(payload.employee_id))
...
@agent_reserve_router.put("/employee/pending-reserves/agent-request/pay", status_code=200)
async def approve_private_reserve(
    #employee_id: str,
    payload: ApprovesReserveSchema,
    db: Session = Depends(get_db_raw)
...
from fastapi import Query
@agent_reserve_router.get(
    "/employee/pending-reserves/agent-request/approved",
    status_code=200
)
async def get_approved_reserves(
    agent_id: Optional[str] = Query(None),
    db: Session = Depends(get_db_raw)
):
    """
    Get APPROVED reservations (awaiting payment)
    and return CV details grouped like get_reserved_cvs.
    """

    # Base query for approved reserves
    ...
    if agent_id:
        query = query.filter(
            RecruitmentAgentPrivateReserveModel.employee_id == agent_id
        )

    approved_reserves = query.all()
    grouped_data = {}

    for r in approved_reserves:

        reserve_id = str(r.id)

        # ================================
        # GET CV MODEL (EMPLOYEE)
        # ================================
        cv = db.query(CVModel).filter(
            CVModel.user_id == r.employee_id
        ).first()
        ...
        if r.agent_id:
            ...
            if agent:
                agent_name = (
                    getattr(agent, "english_full_name", None) or
                    f"{agent.first_name or ''} {agent.last_name or ''}".strip()
                )
        ...
        if r.recruitment_id:
            ...
            if recruiter:
                recruiter_name = (
                    getattr(recruiter, "english_full_name", None) or
        ...
        # ================================
        company = db.query(CompanyInfoModel).filter(
            CompanyInfoModel.user_id == r.recruitment_id
        ).first()
        ...
        # ================================
        # GROUPING (just like get_reserved_cvs)
        # ================================
        if reserve_id not in grouped_data:
            grouped_data[reserve_id] = {
                "reserve_id": r.id,
        ...
        if cv_data:
            grouped_data[reserve_id]["cvs"].append(cv_data)

    return {
        "status_code": 200,
        "message": "Approved reservations with CVs fetched successfully",
        "error": False,
        "count": len(grouped_data),
        "data": list(grouped_data.values())
    }
...
import uuid
import httpx
@agent_reserve_router.post(
    "/reserve-set-agent-accepting-reservation-request-pay",
    status_code=200
)
async def reserve_cv(
    ...
    merchant_ref = f"RSV-{uuid.uuid4().hex[:12]}"
    ...
    payload = {
        ...
        "shopperResultUrl": "https://marrir.com/reserve-set-agent-accepting-reservation-request-pay",
    ...
    hyperpay_response = response.json()

    if response.status_code != 200 or "id" not in hyperpay_response:
        raise HTTPException(
            status_code=400,
            detail={
                "message": "HyperPay checkout creation failed",
                "hyperpay_response": hyperpay_response,
            },
        )

    checkout_id = hyperpay_response["id"]

    # ✅ Create invoice (NO undefined variables)
    invoice = InvoiceModel(
        reference=merchant_ref,     # ✅ store YOUR reference, not checkout id
        buyer_id=user.id,
        amount=amount,
        status="pending",
        type="reserve",
    ...
    return {
        ...
        "data": {
            ...
            "merchant_transaction_id": merchant_ref,
            "amount": amount,
            "currency": "AED",
        },
    }
...
@agent_reserve_router.post("/packages/callback/hyper")

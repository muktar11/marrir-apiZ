...
recruiter_reserve_employeer_router_prefix = "/api/v1/recruiter_reserve_employeer"
...
async def get_promoted_cvs(
    ...
    if user.role == "recruitment":
        ...
        query = query.filter(
            or_(
                # ✅ employees + employers always visible
                CreatorUser.role.in_(["employee", "employer"]),

                # ✅ agents only if approved relation exists
                and_(
    ...
    # =========================
        ...
        query = query.filter(
            and_(
                # ✅ ONLY CVs created by recruitment users
...
async def get_accepted_reserves_by_role(
    ...
    if role == "recruiter":
        query = query.filter(
            RecruitmentAgentPrivateReserveModel.recruitment_id == user_uuid_new
        )
    elif role == "agent":
        query = query.filter(
            RecruitmentAgentPrivateReserveModel.agent_id == cast(user_uuid, pgUUID)

...
#for agent to retireve approved requests coming from employee, which have no recruitment_id and no employeer_id, but have agent_id, and status is accepted
@recruiter_reserve_employeer_router.get(
    "/agent/my-reserve-requests-all",
    status_code=200
)
async def get_agent_reserve_requests(
    agent_id: str,
    db: Session = Depends(get_db_raw)
):
    ...
    reserves = (
        ...
        .filter(
            ...
            RecruitmentAgentPrivateReserveModel.agent_id == agent_uuid,
            RecruitmentAgentPrivateReserveModel.status == "accepted",
            RecruitmentAgentPrivateReserveModel.recruitment_id.is_(None),
    ...
    return {
        "status_code": 200,
        "message": "Agent reservation requests fetched successfully",
        "error": False,
        "count": len(data),
        "data": data
    }
